...
94.20 or "$94" — so a figure counts as grounded when the source contains any
number equal to it within rounding at the precision the REPORT used. What
this catches is not sloppy rounding but invention: prices, percentages, and
ratios that appear in no input at all.
"""
...
def check_figures(report_text: str, source_text: str) -> FigureCheckResult:
    ...
    for m in _FIGURE_RE.finditer(report_text):
        ...
        if value is None:
            continue
        if _decimals(raw) == 0 and abs(value) < _MIN_BARE_INT:
            continue
        checked += 1
